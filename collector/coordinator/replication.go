...
func (coordinator *ReplicationCoordinator) Run() error {
	...
	case utils.VarSyncModeAll:
		if conf.Options.FullSyncReaderOplogStoreDisk {
			l.Logger.Info("run parallel document oplog")
			if err = coordinator.parallelDocumentOplog(fullBeginTs); err != nil {
				return err
			}
		} else {
			l.Logger.Info("run serialize document oplog")
	...
	default:
		l.Logger.Errorf("unknown sync mode %v", conf.Options.SyncMode)
...
func (coordinator *ReplicationCoordinator) sanitizeMongoDB() error {
	...
	if conn, err = utils.NewMongoCommunityConn(checkpointStorageUrl, utils.VarMongoConnectModePrimary, true,
		...
		l.Logger.Errorf("Connect checkpointStorageUrl[%v] error[%v]. Please add primary node into 'mongo_urls' if 'context.storage.url' is empty",
			checkpointStorageUrl, err)
		return err
	}
	conn.Close()
	...
	for i, src := range coordinator.MongoD {
		if conn, err = utils.NewMongoCommunityConn(src.URL, conf.Options.MongoConnectMode, true,
			...
			l.Logger.Errorf("Connect mongo server error. %v, url : %s. "+
		...
		if conf.Options.SyncMode != utils.VarSyncModeFull &&
			...
			l.Logger.Error("There has no oplog collection in mongo db server")
			conn.Close()
		...
		if _, exist := rs[rsName]; exist {
			l.Logger.Errorf("There has duplicate replica set name : %s", rsName)
			conn.Close()
...
func (coordinator *ReplicationCoordinator) parallelDocumentOplog(fullBeginTs interface{}) error {
	...
	nimo.GoRoutine(func() {
		defer docWg.Done()
		if err := coordinator.startDocumentReplication(); err != nil {
			docError = fmt.Errorf("start document replication failed: %v", err)
			l.Logger.Errorf("err: %v", docError)
			return
		}
		l.Logger.Info("------------------------full sync done!------------------------")
	})
	...
	if err := coordinator.startOplogReplication(fullBeginTs, int64(0), nil); err != nil {
		err = fmt.Errorf("start oplog replication failed: %v", err)
		l.Logger.Errorf("err: %v", err)
		return err
	}
